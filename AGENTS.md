...
- `openvibes-collectors` — `collect_processes` (`process.names`, `process.count`; canonical semantics, privileges, and all-or-nothing failure policy in `processes.rs` docs). Linux reads `/proc` with std; Windows/macOS use `sysinfo` (target-only dependency, `Process::kill*` banned in `clippy.toml`). `collect_packages` + `package_facts` (`package.names`, `package.count`, and the `InstalledPackage` inventory; Linux only: RPM 4.16+ SQLite database parsed as RPM headers in `packages/rpm.rs`, dpkg status in `packages/dpkg.rs`; other OSes `unsupported`). `collect_ports` (`port.{tcp,udp}.{exposed,local,listeners}` string lists and `port.{tcp,udp}.exposed.count`; Linux `/proc/net/{tcp,tcp6,udp,udp6}`, native-endian address words; exposed = non-loopback bind address, not firewall reachability; other OSes `unsupported`). Also `hostname()` for export files. String-list facts are sorted and unique (≤ `fact_list_items`, 10,000), and CEL `in` is a binary search.
- `openvibes-agent` — the binary is `openvibes-agent <config.toml>`: `load_config` reads a size-bounded TOML file (`deny_unknown_fields`, absolute paths only) and `Service::tick` runs once a minute (identity → renewal → heartbeat → one delivery batch). `Service::scan_if_due` (`scan.rs`) runs before it: every `scan_interval_seconds` it resolves each configured `[[rule_sets]]` bundle (candidates: `/v1/rule-bundle` on the distribution service when configured and enrolled, then the local `bundle_file`; each goes loader against the `RuleStore` floor → accept; if none is accepted, the re-verified stored bundle), collects facts, evaluates, and enqueues matches. Finding IDs use `install_id`, so scanning needs no enrollment. A failed renewal is reported in `TickReport` but never blocks delivery while the certificate is still valid. The agent API port defaults to 18423 (`DEFAULT_PLATFORM_PORT`); the platform GUI on 443 is separate. `tests/vertical_slice.rs` is the rule-path end-to-end test (signed rule → evaluation → queue → mock platform). Also a library holding the identity lifecycle: `load_or_enroll` loads the stored identity or enrolls with a token, storing it only after the issued chain parses; `renew_if_due` rotates the key at two thirds of the locally measured lifetime and refuses a renewal for another agent; `forget_if_revoked` deletes the identity only on the structured `TransportError::IdentityRevoked`, never a bare 401/403. `tests/enrollment.rs` is the Milestone 4 end-to-end test (enroll → restart → mTLS delivery → renewal → revocation → re-enrollment).
...
- CI is `.github/workflows/ci.yml`: a lint gate on Ubuntu (fmt, clippy, docs, `cargo audit`), then clippy and tests on Ubuntu, Windows, and macOS, so `cfg(windows)`/`cfg(target_os = "macos")` code gets compiled and run. Every action in a workflow must be pinned to a full commit SHA with the version in a comment. License policy and target builds are still pending (`workflow.md`).
