...
- `openvibes-transport` — `PlatformClient` (blocking `ureq` over `rustls`/`ring`, ADR-0005): TLS 1.3-only provider, pinned platform CA roots, `max_redirects(0)` with non-2xx mapped to `Rejected`, explicit proxy only, timeouts and body limits from `ResourceLimits`. `HostKey` generates the P-256 key and CSR (`rcgen`); `ClientIdentity` holds the issued chain for mTLS. The HTTP API is specified in `docs/contracts-v1.md`. `tests/platform.rs` runs against the `openvibes-testkit` mock platform.
- `openvibes-collectors` — scaffold only (descriptor stub).
- `openvibes-agent` — the binary just prints component names; `tests/vertical_slice.rs` is the rule-path end-to-end test (signed rule → evaluation → queue → mock platform). Also a library: `load_or_enroll` loads the stored identity or enrolls with a token, storing it only after the issued chain parses. `tests/enrollment.rs` is the Milestone 4 end-to-end test (enroll → restart → mTLS delivery).
