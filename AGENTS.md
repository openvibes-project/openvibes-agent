...
- `openvibes-collectors` — `collect_processes` (`process.names`, `process.count`; canonical semantics, privileges, and all-or-nothing failure policy in `processes.rs` docs). Linux reads `/proc` with std; Windows/macOS use `sysinfo` (target-only dependency, `Process::kill*` banned in `clippy.toml`). Also `hostname()` for export files.
